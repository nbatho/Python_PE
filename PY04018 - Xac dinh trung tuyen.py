class Diem:
    def __init__(self,id,ten,ma,tin,cm):
        ...
        self.tongdiem = 0
        if ma[1] == "1":
            self.tongdiem = 2.0 + tin*2 + cm
        elif ma[1] == "2":
            self.tongdiem = 1.5 + tin*2 + cm
        elif ma[1] == "3":
            self.tongdiem = 1.0 + tin*2 + cm
        elif ma[1] == "4":
            self.tongdiem = 0.0 + tin*2 + cm
        if self.tongdiem < 18: self.TT = "LOAI"
        else: self.TT = "TRUNG TUYEN"
    def info(self):
        print(self.mgv,self.ten,self.mon,self.tongdiem,self.TT)
a = []
